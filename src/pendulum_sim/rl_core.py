...
class LIGOPendulumEnv(gym.Env):
    ...
    def __init__(self):
        """Initialize action/observation spaces and per-episode state holders."""
        super().__init__()
    ...
    def _get_obs(self):
        """Return normalized observation vector for current simulator state."""
        return build_normalized_obs(self.state)

    def reset(self, seed=None, options=None):
        """Reset environment state and pre-generate disturbance sequence."""
    ...
    def step(self, action):
        """Advance one time step using current action and disturbance sample."""
...
class WandbRolloutLogger(BaseCallback):
    ...
    def __init__(self, wandb_run, verbose=0):
        """Store W&B run handle used for callback logging."""
        super().__init__(verbose)
        self.wandb_run = wandb_run

    def _on_rollout_end(self) -> None:
        """Append reward/timestep history after each rollout."""
    ...
    def _on_step(self) -> bool:
        """Continue training loop without interruption."""
        return True

class ProgressLogger(BaseCallback):
    """SB3 callback that stores/prints rollout-level reward progress."""
    def __init__(self, verbose=0):
        """Initialize reward/history buffers used for progress plots."""
        super().__init__(verbose)
        self.first_rew      = None
        self.reward_history = []
        self.steps_history  = []
    ...
    def _on_step(self) -> bool:
        """Capture first rollout reward baseline during training."""
