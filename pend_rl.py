...
NOISE_FMIN = 0.1     # Hz
NOISE_FMAX = 5.0     # Hz
# reward shaping: frequency-domain objective from research notes
REWARD_WINDOW_SEC = float(os.getenv("REWARD_WINDOW_SEC", "5.0"))
REWARD_EVERY_STEPS = int(os.getenv("REWARD_EVERY_STEPS", "20"))
FORCE_REF_10_30 = float(os.getenv("FORCE_REF_10_30", "0.01"))
STABILITY_RATIO_LIMIT = float(os.getenv("STABILITY_RATIO_LIMIT", "3.0"))
STABILITY_PENALTY = float(os.getenv("STABILITY_PENALTY", "1.0"))
...
class LIGOPendulumEnv(gym.Env):
    def __init__(self):
        ...
        self.x2_hist = []
        self.f_hist = []
        self.reward_window = max(128, int(REWARD_WINDOW_SEC / self.dt))
        self.err_ref_0_5 = 1e-12
        self.err_ref_5_10 = 1e-12
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.noise_enabled:
            ...
            self.noise_seq = np.zeros(N_STEPS + 10, dtype=np.float32)
        self.err_ref_0_5, self.err_ref_5_10 = self._compute_passive_reference_bands(self.noise_seq)
    ...
    def _compute_passive_reference_bands(self, noise_seq):
        state = np.zeros(4, dtype=np.float32)
        x2_hist = []
        for i in range(N_STEPS):
            x_p_ddot = float(noise_seq[i])
            state = state + equations_of_motion(state, x_p_ddot, 0.0) * self.dt
            x2 = L1 * np.sin(state[0]) + L2 * np.sin(state[1])
            x2_hist.append(x2)
        x2_arr = np.asarray(x2_hist)
        ref0 = self._band_mean_asd(x2_arr, 0.0, 5.0)
        ref1 = self._band_mean_asd(x2_arr, 5.0, 10.0)
        return max(ref0, 1e-12), max(ref1, 1e-12)
    ...
    def step(self, action):
        ...
        reward = 0.0
        if (self.current_step % REWARD_EVERY_STEPS == 0) and (len(self.x2_hist) >= self.reward_window):
            x_seg = np.asarray(self.x2_hist[-self.reward_window:])
            f_seg = np.asarray(self.f_hist[-self.reward_window:])
            err_0_5 = self._band_mean_asd(x_seg, 0.0, 5.0)
            err_5_10 = self._band_mean_asd(x_seg, 5.0, 10.0)
            ctrl_10_30 = self._band_mean_asd(f_seg, 10.0, 30.0)

            err_ratio = err_0_5 / self.err_ref_0_5
            ctrl_ratio = ctrl_10_30 / max(FORCE_REF_10_30, 1e-12)
            reward = -(np.log1p(err_ratio ** 2) * np.log1p(ctrl_ratio ** 2))

            stability_ratio = err_5_10 / self.err_ref_5_10
            if stability_ratio > STABILITY_RATIO_LIMIT:
                reward -= STABILITY_PENALTY * np.log1p((stability_ratio / STABILITY_RATIO_LIMIT) ** 2)
